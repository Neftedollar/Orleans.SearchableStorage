...
Each external fixture also runs one resource-isolation cleanup case without starting another cluster. PostgreSQL removes a populated owned schema while a populated foreign schema survives, Redis removes only the selected service id's state keys, and Azure Blob removes only the selected container. The Redis test obtains its key suffix from a state written by the official provider before cloning owned and unrelated sentinels, so it fails if provider and fixture namespace formats diverge. `try`/`finally` cleanup removes every sentinel and the provider-created probe even when an assertion fails.
