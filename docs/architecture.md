...
External fixture initialization treats backend preparation, cluster construction, and deployment as one guarded lifecycle. Any failure attempts to stop a constructed cluster and clean the backend while preserving the original exception; secondary stop and cleanup failures are attached to that exception under stable diagnostic keys. A cleanup attempt is final only after the cluster has stopped, or when cluster construction never completed. If stopping fails, cleanup remains best-effort and pending so a later disposal repeats it after a successful stop, removing writes which could have raced the first attempt. Deterministic lifecycle tests cover preparation, construction, deployment, stop, cleanup, and retry paths without environment-variable mutation or live infrastructure.

Provider cleanup is deliberately namespace-scoped and tested against unrelated sentinels. PostgreSQL drops only its quoted schema, Redis deletes only keys under the selected Orleans service id's `state` prefix, and Azure Blob deletes only its generated container. The Redis case first writes through the official Orleans provider and derives both cleanup sentinels from the resulting physical key; this guards the cleanup contract against provider key-format drift. These checks run against the real external service already used by the fixture and do not deploy a second Orleans cluster.
